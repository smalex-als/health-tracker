package meal

import (
	"fmt"
	"net/http"
	"testing"
	"time"

	"github.com/gin-gonic/gin"
	"github.com/smalex-als/expense-tracker/server/apptest"
	"github.com/smalex-als/health-tracker/server/common"
	"github.com/smalex-als/health-tracker/server/dao"

	"google.golang.org/appengine"
	"google.golang.org/appengine/aetest"
	"google.golang.org/appengine/datastore"
)

func TestInit(t *testing.T) {
	r := gin.New()
	register := &common.HandlerRegister{r}
	(&MealRemoteService{}).Register(register)
	http.Handle("/", r)

	inst, err := aetest.NewInstance(&aetest.Options{StronglyConsistentDatastore: true})
	if err != nil {
		t.Fatal(err)
	}
	defer inst.Close()
	req, _ := inst.NewRequest("GET", "/", nil)
	ctx := appengine.NewContext(req)

	users := []*common.User{
		{Id: 1, Username: "admin", Enabled: true, RoleId: common.ROLE_ADMIN, EmailConfirmed: true},
		{Id: 2, Username: "manager", Enabled: true, RoleId: common.ROLE_MANAGER, EmailConfirmed: true},
		{Id: 3, Username: "smalex", Enabled: true, RoleId: common.ROLE_USER, EmailConfirmed: true},
		{Id: 4, Username: "admin2", Enabled: true, RoleId: common.ROLE_ADMIN, EmailConfirmed: true},
	}
	keys := make([]*datastore.Key, len(users))
	for i, u := range users {
		keys[i] = datastore.NewKey(ctx, "User", "", u.Id, nil)
	}
	if _, err := datastore.PutMulti(ctx, keys, users); err != nil {
		panic(err)
	}
	types := []*MealType{
		{1, "Breakfast"},
		{2, "Lunch"},
		{3, "Dinner"},
		{4, "Snack"},
	}
	keys = make([]*datastore.Key, len(types))
	for i, exType := range types {
		keys[i] = datastore.NewKey(ctx, "MealType", "", exType.Id, nil)
	}
	if _, err := datastore.PutMulti(ctx, keys, types); err != nil {
		panic(err)
	}

	userAdmin, _ := common.CreateToken("admin", 1, common.ROLE_ADMIN, 1)
	userManager, _ := common.CreateToken("manager", 2, common.ROLE_MANAGER, 1)
	userOrdinary, _ := common.CreateToken("smalex", 3, common.ROLE_USER, 1)
	userAdmin2, _ := common.CreateToken("admin2", 4, common.ROLE_ADMIN, 1)

	const mealURL = "/v1/meals/"
	commands := []*apptest.TestCommand{
		{
			Name: "admin create meal",
			Href: mealURL,
			Src: &Meal{
				Amount:      199,
				Description: "Coffee very tasty",
				Date:        time.Now(),
			},
			Dst:   &dao.CommonPutResp{},
			Code:  200,
			Token: userAdmin,
			Validate: func(t *testing.T, cmd *apptest.TestCommand) {
				if cmd.Dst != nil {
					resp := (cmd.Dst).(*dao.CommonPutResp)
					values := (resp.Data).(map[string]interface{})
					if values["id"] != "1" {
						t.Fatal("expected id = 1")
					}
					if values["description"] != "Coffee very tasty" {
						t.Fatal("expected comment = 'Coffee very tasty'")
					}
					fmt.Printf("cmd.Dst = %+v\n", values)
				}
			},
		},
		{
			Name: "admin can update his own meal",
			Href: mealURL,
			Src: &Meal{
				Id:          1,
				Amount:      299,
				Description: "Coffee",
				Date:        time.Now(),
			},
			Code:  200,
			Token: userAdmin,
		},
		{
			Name: "manager create meal",
			Href: mealURL,
			Src: &Meal{
				Amount:      399,
				Description: "Tea and biscit",
				Date:        time.Now(),
			},
			Code:  200,
			Token: userManager,
		},
		{
			Name: "manager can update his own meal",
			Href: mealURL,
			Src: &Meal{
				Id:          2,
				Amount:      499,
				Description: "Tea and biscit",
				Date:        time.Now(),
			},
			Code:  200,
			Token: userManager,
		},
		{
			Name: "admin can update any meal",
			Href: mealURL,
			Src: &Meal{
				Id:          2,
				Amount:      499,
				Description: "Tea and biscit",
				Date:        time.Now(),
			},
			Code:  200,
			Token: userAdmin,
		},
		{
			Name:         "meal not found",
			Method:       "GET",
			Href:         mealURL + "77777",
			Code:         404,
			ErrorMessage: common.ErrNotFound.Message(),
			Token:        userAdmin,
		},
		{
			Name:   "admin get all meals",
			Method: "GET",
			Href:   mealURL,
			Code:   200,
			Token:  userAdmin,
		},
		{
			Name:   "admin get all meals with query",
			Method: "GET",
			Href:   mealURL + "?query=tea",
			Code:   200,
			Token:  userAdmin,
		},
		{
			Name:   "user get meal form",
			Method: "GET",
			Href:   "/v1/meals-form/",
			Code:   200,
			Token:  userManager,
		},
		{
			Name:   "user get all meals",
			Method: "GET",
			Href:   mealURL,
			Code:   200,
			Token:  userManager,
		},
		{
			Name:   "user get all",
			Method: "GET",
			Href:   mealURL,
			Code:   401,
		},
		{
			Name:   "user get all meals with query",
			Method: "GET",
			Href:   mealURL + "?query=tea&date=" + time.Now().Format("2006-01-02"),
			Code:   200,
			Token:  userManager,
		},
		{
			Name:   "user get stats",
			Method: "GET",
			Href:   "/v1/meals-stats/",
			Code:   200,
			Token:  userManager,
		},
		{
			Name:   "admin get meal his own",
			Method: "GET",
			Href:   mealURL + "1",
			Code:   200,
			Token:  userAdmin,
		},
		{
			Name:   "admin get meal any meal",
			Method: "GET",
			Href:   mealURL + "2",
			Code:   200,
			Token:  userAdmin,
		},
		{
			Name:   "admin2 get meal any meal",
			Method: "GET",
			Href:   mealURL + "1",
			Code:   200,
			Token:  userAdmin2,
		},
		{
			Name:         "manager cannot see meal if he is not owner",
			Method:       "GET",
			Href:         mealURL + "1",
			Code:         403,
			ErrorMessage: common.ErrPermissionDenied.Message(),
			Token:        userManager,
		},
		{
			Name: "manager cannot update meal if he is not owner",
			Href: mealURL,
			Src: &Meal{
				Id:          1,
				Amount:      599,
				Description: "Coffee",
				Date:        time.Now(),
			},
			Code:         403,
			ErrorMessage: common.ErrPermissionDenied.Message(),
			Token:        userManager,
		},
		{
			Name:         "user cannot get meal if he is not owner",
			Method:       "GET",
			Href:         mealURL + "1",
			Code:         403,
			ErrorMessage: common.ErrPermissionDenied.Message(),
			Token:        userOrdinary,
		},
		{
			Name:         "manager cannot delete meal if he is not owner",
			Method:       "DELETE",
			Href:         mealURL + "1",
			Code:         403,
			ErrorMessage: common.ErrPermissionDenied.Message(),
			Token:        userManager,
		},
		{
			Name:         "user cannot delete meal if he is not owner",
			Method:       "DELETE",
			Href:         mealURL + "1",
			Code:         403,
			ErrorMessage: common.ErrPermissionDenied.Message(),
			Token:        userOrdinary,
		},
		{
			Name:   "admin delete meal his own meal",
			Method: "DELETE",
			Href:   mealURL + "1",
			Code:   200,
			Token:  userAdmin,
		},
		{
			Name:   "manager delete meal his own meal",
			Method: "DELETE",
			Href:   mealURL + "2",
			Code:   200,
			Token:  userManager,
		},
		{
			Name:         "admin cannot delete meal if it is not exists",
			Method:       "DELETE",
			Href:         mealURL + "7777",
			Code:         404,
			ErrorMessage: common.ErrNotFound.Message(),
			Token:        userAdmin,
		},
		{
			Name:   "if user is not logged he cannot view list of meals",
			Method: "GET",
			Href:   mealURL,
			Code:   401,
		},
		{
			Name:   "if user is not logged he cannot view meal",
			Method: "GET",
			Href:   mealURL + "1",
			Code:   401,
		},
	}
	apptest.CommonApiRunnerAll(r, t, inst, commands)
}
