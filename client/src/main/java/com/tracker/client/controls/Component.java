package com.tracker.client.controls;
...
public class Component {
  ...
  public void addChild(Component child, boolean render) {
    addChildAt(child, getChildCount(), render);
  }

  public void addChildAt(Component child, int index, boolean render) {
    if (children == null) {
    ...
    if (child.isInDocument() && this.isInDocument() 
      ...
      Element siblingEl = null;
      if (index + 1 < getChildCount()) {
        Component sibling = getChildAt(index + 1);
        siblingEl = sibling.getElement();
      }
      child.renderInternal(this.getContentElement(), siblingEl);
